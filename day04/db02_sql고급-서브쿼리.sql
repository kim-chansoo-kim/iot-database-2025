-- 서브쿼리 고급
-- SELECT 서브쿼리 - 스칼라값(단일행, 단일열) 한 컬럼당 데이터는 1개
-- 4-12 : 마당서점의 고객별 판매액을 보이시오(고객이름과 고객별(GROUP BY) 판매액을 출력).
SELECT (SELECT name
          FROM Customer AS c
		 WHERE custid = o.custid) AS '고객이름', SUM(o.saleprice) AS '고객별 판매액'
  FROM Orders AS o
 GROUP BY o.custid;
...
-- 4-15 : Orders 테이블의 평균 주문금액 이하의 주문에 대해 주문번호와 금액을 나타내시오.
...
SELECT orderid AS '주문번호', saleprice AS '금액'
  FROM Orders
